...
// Ear-clipping triangulation for simple 2D polygons, shipped alongside Polygon so UnityX.Geometry is
// self-contained (no external triangulation dependency). Only the Vector2 path is included — the whole
// project triangulates from Vector2 vertex rings. Swap the body here for a library if you need robustness
// on self-intersecting/holed polygons. Note: _indicesScratch is shared static state, so GenerateIndices
