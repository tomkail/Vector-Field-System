...
namespace UnityX.SceneManagement {
	...
	public class RuntimeSceneSetLoader : MonoSingleton<RuntimeSceneSetLoader> {
		...
		private async Awaitable LoadSceneSetupAsync() {
			var startTime = Time.realtimeSinceStartup;
			currentLevelSetLoadTask.AssignTasks();

			if (currentLevelSetLoadTask.unloadTasks.Count != 0)
				OnWillUnload?.Invoke();

			await currentLevelSetLoadTask.LoadSceneSetupAsync(() => {
				...
				if (!currentLevelSetLoadTask.cancelled)
					OnCompleteTask?.Invoke(currentLevelSetLoadTask);
		...
		private void OnPreCompleteSceneSetLoad() {
			...
			if(pendingLevelSetLoadTasks.Count != 0) {
		...
		private void OnPostCompleteSceneSetLoad() {
			if(currentLevelSetLoadTask == null && pendingLevelSetLoadTasks.Count == 0) {
		...
		// Broadcasts a message to every ISceneSetMessageReceiver in a loaded scene. Interface-based
		// (no reflection, works in edit and play mode); receivers dispatch on the message string.
		public static void BroadcastMessageScene (UnityEngine.SceneManagement.Scene scene, string message, object argument = null) {
			if (!scene.isLoaded) {
				Debug.LogWarning("Tried to broadcast '" + message + "' to scene '" + scene.name + "', but scene is not loaded.");
				return;
			}
			foreach (GameObject root in scene.GetRootGameObjects().OrderBy(x => x.transform.GetSiblingIndex())) {
				foreach (var receiver in root.GetComponentsInChildren<ISceneSetMessageReceiver>(true))
					receiver.OnSceneSetMessage(message, argument);
			}
		}
	}
}
