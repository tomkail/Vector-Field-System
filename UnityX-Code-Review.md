...
UI-29. `UIGradient.cs:228-230` — the radial-gradient distance uses `Mathf.Pow(x,2f)` to square; equivalent to `x*x` / a `Vector2(dx*width, dy*height).magnitude`. *(Left as-is: correct, just a micro-inefficiency — a `Pow` call per vertex. Explained, not worth a behavioural edit.)*
UI-30. `Line/AdvancedUILineRenderer.cs:177-198` — `AddQuad` vs `VertexHelper.AddUIVertexQuad`. *(Not a true reimplementation, left as-is: `AddUIVertexQuad` triangulates the quad along the 0–2 diagonal `(0,1,2)+(2,3,0)`; the local `AddQuad` splits along the 1–3 diagonal `(1,2,3)+(3,0,1)`. Same quad only when planar/convex — the diagonal choice is a deliberate difference, not accidental duplication.)*
...
UI-36. `Line/AdvancedUILineRenderer.cs` vs `UILineRenderer.cs` — duplicated miter/bevel join block + duplicated `MIN_MITER_JOIN`/`MIN_BEVEL_NICE_JOIN` constants. *(Deferred: the two blocks are NOT identical — different loop/wrap logic (Advanced supports loop joins via `startIndex`/`endIndex`), different miter-distance source (`LineThickness` vs `Vector2.Distance` between edge verts), and different quad emission (`AddUIVertexQuad` vs the local `AddQuad`, opposite triangulation diagonal per UI-30). A shared helper is feasible but is a mesh-generation change with no in-editor/visual verification available right now — do this once the community MCP/editor is up.)*
...
UI-41. `RoundRect/RoundRect.cs` — `AddMiddle` (503) never called; `_debugBool` unused; unused `corner1/2` (178); geometry methods take `vertices`/`indices` params but use fields instead. *(Intent: the geometry builders (`MakeRoundRectOutlineGeometry`/`AddCorner`/`AddEdge`/`AddMiddle`) were written to be buffer-agnostic — take the target `vertices`/`indices` lists as params — but the bodies hardwire the `vertexBuffer`/`indexBuffer` instance fields instead, so the params are dead (callers always pass those same fields). `corner1/2` in `CalcOuterGeom` are leftovers from when `RoundRectOuterGeom` presumably carried the corners; the struct now only holds `rect`+`radius`. `_debugBool` is a stray serialized debug toggle. `AddMiddle` was superseded by the `MakeHardQuad(innerRect)` fill. All dead/cosmetic — safe to tidy, not yet done.)*
...
- **UI-32** `UI Imposter/UIImposterRenderer.cs` — the local `CreateEncapsulating(Vector3[])` AABB builder now delegates to `BoundsX.CreateEncapsulating` (it was a verbatim copy). *Correction:* the review's "latent bug in the `else if` min/max chain" is a **false positive** — per-axis `if (< min) … else if (> max) …` is correct (a value can't be below the running min *and* above the running max within one axis), and it's the same pattern used throughout `BoundsX`/`RectX`. The `Vector2[]`→`Rect` overload is left as-is (2D, justified).
...
- ⚠️ **Not compile-verified in-editor** (community "MCP for Unity" server down). All changes are behaviour-preserving refactors/dead-code removal.
