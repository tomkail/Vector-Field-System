// Lit normal-flow water (URP): the same ping-pong flow map as WaterFlowMap, but it flows a HEIGHTFIELD (the water
...
Shader "Vector Fields/Water Flow Lit" {
    ...
    SubShader {
        ...
        Pass {
            ...
            // One ping-pong flow layer -> scalar height (see WaterFlowMap for the ping-pong rationale).
            ...
            half4 frag (Varyings input) : SV_Target {
                float2 vel = -1.0 * (SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv).rg - 0.5); // decode field
