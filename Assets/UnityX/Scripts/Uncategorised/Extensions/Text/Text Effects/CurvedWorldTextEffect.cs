...
public class CurvedWorldTextEffect : BaseTextMeshProEffect {
    protected override void OnPreRenderText(TMP_TextInfo textInfo) {
        ...
        // The transform is invariant across all vertices, so compute it (and its inverse) once.
        ...
        foreach (var characterInfo in textInfo.characterInfo) {
            ...
            vertices[vertexIndex + 0] = PerformVertexEffect(m, mInverse, vertices[vertexIndex + 0], out float alpha0);
            vertices[vertexIndex + 1] = PerformVertexEffect(m, mInverse, vertices[vertexIndex + 1], out float alpha1);
            vertices[vertexIndex + 2] = PerformVertexEffect(m, mInverse, vertices[vertexIndex + 2], out float alpha2);
            vertices[vertexIndex + 3] = PerformVertexEffect(m, mInverse, vertices[vertexIndex + 3], out float alpha3);
    ...
    static Color32 MultiplyAlpha(Color32 color, double alpha) {
        var a = color.a * alpha;
    ...
    Vector3 PerformVertexEffect(Matrix4x4 m, Matrix4x4 mInverse, Vector3 vertex, out float alpha) {
        var normalized = mInverse.MultiplyPoint3x4(vertex);
