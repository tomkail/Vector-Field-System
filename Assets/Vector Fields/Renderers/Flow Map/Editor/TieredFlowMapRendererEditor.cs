...
// flow styling, and the N speed tiers. The tier editor is an IMGUI slider modelled on Unity's LODGroup editor
// (LODGUI.cs / LODGroupEditor.cs): contiguous colour-coded regions across the speed axis with a "Tier n / %" label,
// draggable boundary handles (hotControl), click-to-select, and a right-click Insert / Delete menu. The selected tier's
// settings are shown beneath the bar. Axis runs 0 (still, left) → 1 (Max Speed, right).
...
public class TieredFlowMapRendererEditor : VectorFieldTextureRendererEditor {
	...
	const int MaxTiers = TieredFlowMapRenderer.MaxTiers;
	...
	protected override void BuildBody(VisualElement root) {
		...
		if (!serializedObject.isEditingMultipleObjects)
			tierSection.Add(new IMGUIContainer(OnTierGUI)); // single-target only; mixed selections fall back to the shared sections
	...
	void OnTierGUI() {
		...
		EditorGUILayout.PropertyField(el.FindPropertyRelative("speed"));
		EditorGUILayout.PropertyField(el.FindPropertyRelative("texture"));
		EditorGUILayout.PropertyField(el.FindPropertyRelative("tiling"));
		EditorGUILayout.PropertyField(el.FindPropertyRelative("flowStrength"));
		EditorGUILayout.PropertyField(el.FindPropertyRelative("flowSpeed"));
	...
	void ShowContextMenu(Rect bar, SerializedProperty tiersProp, int[] order, int n, Vector2 mouse) {
		...
		if (n > 1) menu.AddItem(new GUIContent("Delete Selected Tier"), false, () => DeleteTier(tiersProp, selectedTier));
	...
	void InsertTier(int srcElem, float newTierSpeed, float sourceSpeed) {
		serializedObject.Update();
		var tiers = serializedObject.FindProperty("tiers");
	...
	void DeleteTier(SerializedProperty _, int index) {
		serializedObject.Update();
		var tiers = serializedObject.FindProperty("tiers");
