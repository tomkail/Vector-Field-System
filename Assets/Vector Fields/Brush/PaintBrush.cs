...
public struct PaintBrush<T> {
    public VectorFieldBrushShape shape;   // the spatial profile is value-agnostic, so it's shared as-is
    ...
    public VectorFieldDirectionMode directionMode;   // how a map/directional brush orients (ignored by radial shapes)

    public PaintBrush(VectorFieldBrushShape shape, IBrushOp<T> op, float size = 1.5f, float pressure = 1f,
