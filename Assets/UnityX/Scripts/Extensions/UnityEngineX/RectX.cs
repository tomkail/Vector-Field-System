...
public static class RectX {
	...
	// Creates a new rect from an anchor position, size and pivot.
	public static Rect Create (Vector2 anchorPosition, Vector2 size, Vector2 pivot) {
		return new Rect(anchorPosition.x - size.x * pivot.x, anchorPosition.y - size.y * pivot.y, size.x, size.y);
	}
	...
	/// <summary>
	/// Returns a new rect with a specified size from a pivot point on the original rect.
	/// </summary>
	/// <param name="r">The rect.</param>
	/// <param name="size">New size.</param>
	/// <param name="pivot">Pivot point. 0 is bottom/left, 0.5 is center, 1 is top/right.</param>
	/// <returns>New rect with specified size and pivot.</returns>
	public static Rect WithSize(this Rect r, Vector2 size, Vector2 pivot)
	{
		// Calculate the difference in size
		Vector2 sizeDiff = size - r.size;

		// Calculate the new position based on the pivot
		Vector2 newPosition = new Vector2(
			r.position.x - sizeDiff.x * pivot.x,
			r.position.y - sizeDiff.y * pivot.y
		);

		// Return the new rect with the calculated position and the new size
		return new Rect(newPosition, size);
	}

	public static Rect WithWidth(this Rect r, float width, float pivot = 0) {
		return WithSize(r, new Vector2(width, r.height), new Vector2(pivot, 0.5f));
	}

	public static Rect WithHeight(this Rect r, float height, float pivot = 0){
		return WithSize(r, new Vector2(r.width, height), new Vector2(0.5f, pivot));
	}
	...
	/// <summary>
	/// Returns the closest distance between a point and a rect.
	/// </summary>
	public static float GetDistanceToPoint (Rect rect, Vector2 position) {
		var closestPoint = rect.ClosestPointOnPerimeter(position);
		return (closestPoint - position).magnitude;
	}
	
	/// <summary>
	/// Returns the signed distance between a point and a rect. The result will be negative if the point is inside the rect.
	/// </summary>
	public static float GetDistanceToPointSigned (Rect rect, Vector2 position) {
		var distanceToPoint = GetDistanceToPoint(rect, position);
		return rect.Contains(position) ? -distanceToPoint : distanceToPoint;
	}
