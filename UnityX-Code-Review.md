...
UI-44. `Grid Layout/GridLayout.cs:258-264` — `pivot` param threaded through `CalculatePositionForGridCoord` (and up via `GetPositionForGridCoord`/`GetCenterPositionForGridCoord`/`GetLocal|WorldPositionForGridCoord(coord,pivot)`) but the leaf formula never uses it. *(Needs a decision, not a blind delete: with pivot dead, `GetCenterPositionForGridCoord` (which passes `0.5`) is **identical** to `GetPositionForGridCoord` — so this is most likely an unfinished **centering** feature (a latent bug), not a spare parameter. Removing pivot is also a public-API reduction across ~8 methods. Options: (a) implement the intended `+ itemSize*pivot` offset so centring works; (b) delete the dead pivot chain (breaks public API). Awaiting your call.)*
UI-46. `SLayout/SLayoutAnimation.cs` — two near-identical `ThenAnimateInternal` overloads (differ only in easing-vs-curve). *(Proposal drafted — NOT the public parameterized ctor, which is external API and stays. Awaiting approval before implementing.)*
UI-47. `SLayout/SLayout.cs:70-159` — the `Animate`/`AnimateCustom`/`After` overloads duplicate the same build-and-start block; only the AnimationCurve overload runs edit-mode `CompleteImmediate()`. *(Proposal drafted; the consistent fix changes edit-mode behaviour of the other overloads, so needs your call. Awaiting approval.)*
UI-50. `AbsoluteRectTransformController.cs:34-40` — both `Update()` and `LateUpdate()` call heavy `Refresh()`. *(Answer given — options: dirty-flag so `Refresh()` early-outs when nothing changed, or keep only `LateUpdate` (post-layout) and drive edits via `OnRectTransformDimensionsChange`, which it already overrides. Not yet changed.)*
UI-54. `SLayout/SLayout.cs:809-817` — `GetPivotPos` duplicates inline `pivot * rect.size` math scattered through the layout getters. *(Proposal drafted — delicate SLayout coordinate math, show-first. Awaiting approval.)*
...
- **UI-45** `Grid Layout/Editor/GridLayoutEditor.cs` — replaced the fragile `GetValueFromObject<T>` reflection with the built-in `SerializedProperty.boxedValue` (Unity 2022.1+) at the call site and in `GetBaseProperty<T>`. The legacy `(object, path)` `GetValueFromObject<T>` is public API and left in place (now superseded). *The same `boxedValue` fix applies to the other copies (Property Drawers `EnumFlagDrawer`/`ButtonDrawer`/`BaseIfAttributeDrawer`, `Extensions/…/ReflectionX`+`SerializedPropertyX`) — to be done in their sections (PD-/UEX-).*
...
- ⚠️ **Not compile-verified in-editor** (community "MCP for Unity" server down). Nearly all changes are behaviour-preserving; the two with a runtime footprint worth an in-editor glance are **UI-30** (quad triangulation diagonal — expected identical for convex line quads) and **UI-51** (blur step-size/Shader caching).
