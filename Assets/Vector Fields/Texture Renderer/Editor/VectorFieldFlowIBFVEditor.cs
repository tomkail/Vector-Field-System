...
public class VectorFieldFlowIBFVEditor : VectorFieldQuadEditor {
	...
	protected override void BuildBody(VisualElement root) {
		...
		buffers.Add(VectorFieldInspectorUI.Help("Prototype (IBFV, van Wijk 2002). Material, noise and buffers auto-generate if left empty."));
		buffers.Add(VectorFieldInspectorUI.Field(serializedObject.FindProperty("ibfvMaterial"), "Material",
			"Material using the \"Vector Fields/Vector Field Flow IBFV\" shader. Auto-created if empty."));
