...
namespace UnityX.SceneManagement.Editor {
	...
	public class RuntimeSceneSetLoaderEditor : UnityEditor.Editor {

		RuntimeSceneSetLoader data => (RuntimeSceneSetLoader)target;
	...
	//			includesFileList.elementHeight = 16;
			...
			loadTasksList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
	            ...
	            var label = loadTask.sceneName+" ("+state+")" + (extraStates.Count == 0 ? string.Empty : " ("+string.Join(", ", extraStates.ToArray())+")");
